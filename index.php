<head style=''>
    ...
    <div class='nav-bar' style=''><h1 class='logo' style='font-family: SFProDSB, sans-serif; display: inline;'>	S3Helper<p style='font-family: SFProDL, sans-serif; display: inline; font-size: 15px;'>beta 0.3.4</p></h1></div>
...
while ($row = mysqli_fetch_array($result)) {
 echo "<div loading='lazy' class='watch' style='display: inline-block; z-index: 0;'>
 <img class='image' style='border-radius: 20px; background-image: url(lazy-load.png);' src='/images/$row[imagelink]'>
           <div class='back-color' style=' border-radius: 20px; box-shadow: 0 3px 12px -1px rgba(7,10,25,.2), 0 22px 27px -20px rgba(7,10,25,.2); background-color: $row[basecolor]; margin-right: 0px;'>
            <div class='highlight' style='border-radius: 20px; box-shadow: 0 3px 12px -1px rgba(7,10,25,.2), 0 22px 27px -20px rgba(7,10,25,.2); background-color: $row[highlightcolor]; z-index: 0;'>
            </div>
           </div>
...
function off() {
  document.getElementById('overlay').style.display = 'none';
          ...
          <div>
          <div class='qr-code' onclick='on()'><div style='margin-left: 10px; padding-top: 3%; display: inline-block; pointer: auto;'><img class='qr-icon' src='qrcode.viewfinder.png'><img class='white-qr-icon' src='white.qrcode.viewfinder.png'><p class='qr-button' style='font-family: SFProDSB, sans-serif; display: inline-block;'>Scan QR Code</p></div></div><style>
...
#text{
  ...
  color: black;
  font-family: SFProDR, sans-serif;
  text-align: center;
  z-index: 300;
}
</style>
...
<p class='description' style='font-family: SFProDL, sans-serif; margin left:0px; text-align: center;'>$row[model]</p>
          ...
          <div style='background-color: rgba(157, 157, 158, 0.21); backdrop-filter: saturate(180%) blur(20px); pointer: hover; left: 0; right: 0; top: 0; bottom: 0; z-index: 9999;' id='overlay' onclick='off()'><div style='color: white; pointer: hover;' id='overlay2' onclick='off()'>
  <div style='background-color: blue; z-index: 9999;'></div><div id='text'><p class='qr-heading'>Download Face on iPhone</p><img src='https://api.qrserver.com/v1/create-qr-code/?size=150x150&format=svg&bgcolor=RRGGBB&data=https://watchdatabase.000webhostapp.com/faces/$row[facefilelink]'><p class='qr-subheading'>Scan the QR code above with Camera app on iPhone.</p></div>
